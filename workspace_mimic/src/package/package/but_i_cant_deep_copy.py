...
#Deep Copy of the Pose Object
def deepCopy(lhs, rhs):
...
def traverse(stack, navigator, fromDir):
    ...
    # new_goal.pose.orientation.z = prev_goal.pose.pose.orientation.z
    ...
    #left
    if(fromDir != -1):
        new_goal.pose.position.y = new_goal.pose.position.y + 0.1
        path = navigator.getPath(prev_goal)
        if(path != None):
            print("Path exist!")
            print("Go Left")
            printPose(new_goal)
        ...
        new_goal.pose.position.y = new_goal.pose.position.y - 0.1 #undo the increment
    #######################################################################################
    #DEAD END
