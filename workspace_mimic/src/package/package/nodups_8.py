...
def check_duplicate_then_append(stack ,posObj):
    for jkl in stack:
        xStat = math.isclose(posObj.pose.position.x, jkl.pose.position.x, rel_tol=1e-2)
        yStat = math.isclose(posObj.pose.position.y, jkl.pose.position.y, rel_tol=1e-2)
        if(xStat == False or yStat == False):
            print("No Duplicates!")
            stack.append(posObj)
            return True
...
def traverse(stack, navigator, fromDir):
    ...
    # new_goal = prev_goal - shallow copy
    print("Traverse")
    #top
    if(fromDir != -2):
        ...
        if(path != None):
            ...
            status = check_duplicate_then_append(stack, new_goal)
            if(status == True):
